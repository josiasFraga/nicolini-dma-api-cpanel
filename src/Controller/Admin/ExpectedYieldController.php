	...
	public function add()
	{
		...
		if ($this->request->is('post')) {
			...
			if ( $soma > 100 ) {
				...
				$this->Flash->error(__('O somatório das expectativas deve somar 100. Total='.$soma, 'Expected Yield'));
			} else {

				// Verifica se o usuário selecionou múltiplas lojas
				if (!empty($dados['store_code']) && is_array($dados['store_code'])) {
					$registros = [];

					foreach ($dados['store_code'] as $storeId) {
						$expectedYield = $this->ExpectedYield->newEmptyEntity();

						// Cria um novo array de dados para cada loja
						$registro = $dados;
						$registro['store_code'] = $storeId; // Define a loja para este registro
						
						// Popula a entidade
						$registros[] = $this->ExpectedYield->patchEntity($expectedYield, $registro);
					}

					// Salva todos os registros de uma vez
					if ($this->ExpectedYield->saveMany($registros)) {
						$this->Flash->success(__('Os registros foram salvos com sucesso.'));
